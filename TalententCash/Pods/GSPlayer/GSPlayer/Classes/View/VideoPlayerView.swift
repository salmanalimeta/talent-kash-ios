...
public struct CFilter {
   public let name:String
    public let displayName:String
    public let inputs:KeyValuePairs<String,Any>
   public init(name: String, displayName: String, inputs: KeyValuePairs<String, Any>) {
        self.name = name
...
@objc extension VideoPlayerView {
    ...
    open func load(for url: URL) {
        ...
        self.player = player
        self.playerURL = url
        self.pausedReason = .userInteraction
        self.replayCount = 0
        self.isLoaded = false
...
public extension VideoPlayerView {
    func play(for url: URL,filterName:CFilter,filterIndex:Int) {
            ...
            guard playerURL != url else {
                if(filterName.name != ""){
                    ...
                    let s = player?.currentItem?.asset.tracks(withMediaType: .video).first?.naturalSize ?? .init(width: 400, height: 500)
                    ...
                    player?.currentItem?.videoComposition = AVVideoComposition(asset: (player?.currentItem!.asset)!, applyingCIFiltersWithHandler: { request in
                        ...
                        let filter = CIFilter(name:filterName.name)!
                        filter.setDefaults()
                        filterName.inputs.forEach { (k,v) in
                            filter.setValue(v, forKey: k)
                        }
                        if filterName.name.contains("Distortion"){
                            filter.setValue(CIVector(x: s.width*0.45, y:  s.width*0.4), forKey: "inputCenter")
