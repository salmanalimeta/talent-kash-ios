...
extension HomeVideoViewController:UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.selectSong = self.selectSongVideo
...
//    }
    ...
    private func selectSongVideo(user:User,reel:Reel?){
        
        if reel?.song?.title.isEmpty != true{
            
            if  UserDefaultManager.instance.userType != .guest{
                
                self.startPulseAnimation()
                
                UserDefaults.standard.set(reel?.song?._id, forKey: "song_id")
                if let audioUrl = URL(string: reel?.song?.song ?? "") {
                    ...
                    if FileManager.default.fileExists(atPath: destinationUrl.path) {
                        ...
                        UserDefaults.standard.set(destinationUrl, forKey: "songURL")
                        UserDefaults.standard.set(reel?.song?.title ?? "", forKey: "songName")
                        
                        UserDefaults.standard.set(reel?.song?._id, forKey: "song_id")
                        DispatchQueue.main.async {
                            
                            let st:UIStoryboard = UIStoryboard(name: "CreatingVideo", bundle: nil)
                            let vc = st.instantiateViewController(withIdentifier: "actionMediaViewController") as! actionMediaViewController
                            vc.modalPresentationStyle = .fullScreen
                            self.present(vc, animated: true, completion: nil)
                            NotificationCenter.default.post(name: Notification.Name("songAdded"), object:nil, userInfo: ["song_id":reel?.song?._id ?? "0"])
                        }
            ...
            self.showToast(message: "There is no song available for this reel.")
        }
