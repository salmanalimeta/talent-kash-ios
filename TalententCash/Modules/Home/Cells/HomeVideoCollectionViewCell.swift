...
//    @IBOutlet weak var giftCounter: UILabel!
    ...
    @IBOutlet weak var songStack: UIStackView!
...
//    var delegateHomeVideoVC:HomeVideoViewController!
    ...
    var selectSong:(User,Reel?)->(Void) = {_,_ in }
    // MARK: Lifecycles
...
//        self.imgMusicBtn.makeRounded()
        ...
        let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(self.handleSongTab(_:)))
            songStack.isUserInteractionEnabled = true
            songStack.addGestureRecognizer(tapGestureRecognizer)
...
//        playerView.playerLayer.player?.rate = Float(self.reel?.speed ?? "1") ?? 1.0
    ...
    @objc func handleSongTab(_ gesture: UITapGestureRecognizer){
        
        guard let user = self.reel?.user else {
            return
        }
        self.selectSong(user,self.reel)
